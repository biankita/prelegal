...
export const STANDARD_TERMS: StandardTermsClause[] = [
  {
    number: 1,
...
export const STANDARD_TERMS_FOOTER =
  "Common Paper Mutual Non-Disclosure Agreement (Version 1.0) free to use under CC BY 4.0.";

export function fillClauseBody(
  body: string,
  vars: { governingLaw: string; jurisdiction: string },
): string {
  return body
    .replaceAll("{{governingLaw}}", vars.governingLaw || "[Governing Law]")
    .replaceAll("{{jurisdiction}}", vars.jurisdiction || "[Jurisdiction]");
}
